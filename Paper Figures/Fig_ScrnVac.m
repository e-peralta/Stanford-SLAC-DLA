...
cols(1,:)=[1 1 1]; 
cols(2,:)=[255 205 135]/255;  %light orange
cols(3,:)=[166 160 220]/255; %light purple
cols(4,:)=[205 66 0]/255;    %dark orange
cols(5,:)=[47 08 117]/255;   %dark purple
cols(6,:)=[0 0 0];  
...
vac2=imrotate(vac,-1.5,'crop'); %-1.35
...
% [maxV maxP]=max(mean(screenD'));
% specVac=screenD(maxP,:);

%remove x-rays
screenD(screenD>2*peakAmp0)=0;   %originally 2x
...
screenD2=vac2-ybkgd(1)*ones(size(vac2));
screenD2(screenD2<0)=0;
screenD2(screenD2>2*peakAmp0)=0;

scrnVac2=screenD2;
specVac2=mean(screenD2);
%[maxV maxP]=max(mean(screenD2'));
%specVac2=screenD2(maxP,:);
...
% hold on
hold on
h1=plot(xD,specVac/1000,'x','LineWidth',1,'Color',cols(3,:),'MarkerSize',4);
% hold on
% for i=1:1 %extend to 4 to test the other fitting algorithms (1 was best)
%     [c1, c1std, yfit, ChiSq]=FitSpectrum5c(Y1,i);
%     h1f=plot(xD,yfit/1000,':','LineWidth',1,'Color',col3);%col4)
%     hold on
%     switch i
%         case 1
%             scale1=sqrt(2*log(2));
...
hold on
h2=plot(xD,specVac2/1000,'x','LineWidth',1,'Color',cols(2,:),'MarkerSize',4);
...
for i=1:1 %extend to 4 to test the other fitting algorithms (1 was best)
    [c1, c1std, yfit, ChiSq]=FitSpectrum5c(Y1,i);
    h2f=plot(xD,yfit/1000,'LineWidth',1,'Color',cols(4,:));
    hold on
    switch i
        case 1
            scale1=sqrt(2*log(2));
...
h2=plot(xD,specGla/1000,'LineWidth',2,'Color',col4);%col3)
hold on
...
for i=2:2 %extend to 4 to test the other fitting algorithms (2 was best)
    ...
    h2f=plot(xD,yfit/1000,':','LineWidth',1,'Color',col6);%col5)
    hold on
    switch i
        case 1
            scale1=sqrt(2*log(2));
...
hL=legend([h1,h2,h2f],{'Spectrum','Rotated spectrum','Gauss fit'});
set(hL,'Location','NorthWest','Box','Off')
set([ax1,ax3],'FontSize', 10);
set([hx3,hy1,hy3,hL],'FontSize', 10);
