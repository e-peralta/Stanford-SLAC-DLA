function [cout, coutstd, yfit, ChiSq, ybkgd, ymain, ysignal, FOM, modeList]=FitSpectrum(modef,y,x,shift,tmod,ploton,list)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%    y = data to fit to
%    x = independent variable tied to data to fit
...
%
%  121112, eperalta@slac.stanford.edu (EAP)
%  141118, fixed to be able to work for unevenly spaced data (EAP) and even
%          data with few data points
%
%   *****XXXX      Still need to fix cmin/cmax for mode >4    !!   XXXX**
...
xdata=x(1+buff1:end-buff2);
ydata=y(1+buff1:end-buff2);

xshift=buff1*ones(1,length(x));
...
ybkgd=minY*ones(1,length(ydata));
ydata=ydata-ybkgd;
%ybkgd=minY*ones(1,length(ydata));
...
g2=xdata(g2);  %% NEW FOR LATEST VERSION


if g2-xdata(1)>xdata(end)-g2
    g3=(xdata(end)-g2)/3;
else
    g3=(g2-xdata(1))/3;
end
...
cmin=[min(ydata) xdata(1)-xrange 0 -g1/10];
cmax=[2*max(ydata) xdata(end)+xrange xdata(end) g1/10];
elseif modef==1
%%asymmetric gaussian
...
cmin=[min(ydata) xdata(1)-xrange 0 0 -g1/10];
cmax=[2*max(ydata) xdata(end)+xrange xdata(end) xdata(end)  g1/10];
...
if exist('lsqcurvefit')==2
   ...
   %else
       %[c1,ChiSqX,~,exitflag,~,~,~,c1std]=lsqcurvefitstd(f,guess,1:length(
       %y),y,cmin,cmax,SIG_OPTIONS);       
       [c1,ChiSqX,~,exitflag,~,~,~,c1std]=lsqcurvefitstd(f,guess,xdata,ydata,cmin,cmax,SIG_OPTIONS);       
   %end
   %c1(2)=c1(2)+xmin;
else
...
yfit= f(c1,xdata);
yfit=[zeros(1,buff1),yfit,zeros(1,buff2)];
ymain=[zeros(1,buff1),ymain,zeros(1,buff2)];
ysignal=[zeros(1,buff1),ysignal,zeros(1,buff2)];
...
if ploton
    ...
    plot(xdata,ymain+ybkgd,'r',xdata,ybkgd,'b','LineWidth',2);
    legend ('Raw data','Fit','Background');

    else
    hold on
    plot(xdata,yfit+ybkgd,'g');
    hold on
    plot(xdata,ymain+ybkgd,'r',xdata,ysignal+ybkgd,'b','LineWidth',2);
%     hold on
%     plot(x,y-yfit,'cx');
    hold on
    plot(xdata,ybkgd,'c');
